...
class OCRTool extends Tool {
    constructor(cooldown = 5000, thresh = 0.5) {
        super('OCR', cooldown, thresh);
    }
    ...
    async run(snapshot) {
        ...
        try {
            ...
            snapshot.ocrResult = {text: '', confidence: 0};
        } finally {
        ...
        try {
            const resp = await axios.post(this.url, {
                ...
                messages: [{
                    ...
                    images: [snapshot.screenshot.toString('base64')]
                }],
                stream: false
            });
            ...
            snapshot.llmAnalysis = {...snapshot.llmAnalysis, vision: 'Vision analysis failed.'};
        }
        this.recordRun();
    }
}
...
class TextLLMTool extends Tool {
    ...
    async run(snapshot) {
        ...
        try {
            ...
            snapshot.llmAnalysis = {...snapshot.llmAnalysis, text: 'Text analysis failed.'};
        }
        this.recordRun();
    }
}
...
class ProcessCollector {
    ...
    async collect() {
        ...
        return Array.from(new Map(combined.map(p => [p.pid, {
            ...
            name: p.name,
            cpu: +p.cpu.toFixed(2),
            memory: +(p.memRss / (1024 * 1024)).toFixed(2)
        }])).values());
    }
}
...
class WindowCollector {
    async collect() {
        try {
            ...
            return {title: 'Error', owner: 'Error', processId: -1, bounds: {}};
        }
    }
}
